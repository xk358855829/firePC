...
import iconCar from '../../../static/image/carsslv.png'; //以import的方式导入图片文件
import newiconCar from '../../../static/image/xxx.png'; //以import的方式导入图片文件
...
export default {
  ...
  methods: {
    // async test(){
    //     let result = await this.setMapNow();
    //     let result1 = await this.setMap();
    // },
     setMapNow(){
    ...
    console.log(data)
     axios({
      method: "post",
      url: 'http://39.104.102.213:8796/devices/findDevByCd',
      ...
      if (res.data.resCode == 0) {
        ...
        for (let i = 0; i < res.data.result.length; i++) {
          ...
          let online = res.data.result[i].online;
              ...
              let lng = '';
              let obj = {
                lng: res.data.result[i].amendLongitude, 
                lat: res.data.result[i].amendLatitude,
                id: res.data.result[i].deviceId,
                online: res.data.result[i].online
              };
              let data = {
                devId: deviceId
                // datastreamIds: '2011'
              ...
              axios({
                method: "post",
                url: 'http://39.104.102.213:8796/Datapoints/getBatData',
                data: Qs.stringify(data)
                  ...
                  if(messData==undefined){
                    ...
                    for (let i = 0; i < messData.length; i++) {
                      switch (messData[i].id) {
                        case '2011':watersValue =messData[i].value;obj.stutatype=watersValue
                          break;
                        default: break;
                      }
                    }
                ...
                // }
                // obj.type=watersValue
              })
              // console.log(obj)
               that.maxList.push(obj)
        }
        setTimeout(function(){that.getmap()},5000)
        } else {
     ...
     getmap(){
      console.log(this.maxList)
       document.getElementById('allmap').style.height = document.body.clientHeight - 70 + 'px';
        ...
        // if(that.maxListLength==that.maxList.length){
          // console.log(that.maxList)
      // let that=this;
      // setTimeout(function(){

              let MAX = that.maxList.length;
              let markers = [];
              let pt = null;
              // console.log(MAX)
              for (let i = 0; i < MAX; i++) {
                pt = new BMap.Point(that.maxList[i].lng, that.maxList[i].lat);

                let marker = new BMap.Marker(pt);
                const deviceSize = new BMap.Size(16, 26);
                // console.log(that.maxList[i].type)
                if(that.maxList[i].id=="58812802"){
                  console.log(that.maxList[i])
                  console.log(JSON.stringify(that.maxList[i]));
                  console.log(JSON.parse(JSON.stringify(that.maxList[i])))
                  console.log(that.maxList[i].stutatype)
                }
                if (that.maxList[i].stutatype == 0||that.maxList[i].stutatype==64) {
                ...
                that.addClickHandler('设备编号:' + that.maxList[i].id, marker);
