<template>
	<div id="carMonitor">
		...
		<div class='titleLeft'>
			...
			<el-button class="oyButton" type="success" icon="el-icon-search" @click='mappInput'>查询</el-button>
					...
					<template slot-scope="scope">
				        <el-popover trigger="click" placement="top">
				        	...
				        	<div slot="reference" class="name-wrapper">
				        		<div class="overflowHide" @click="mapp(scope.row)">{{ scope.row.devParaSn}}</div>
				        	</div>
				        </el-popover>
			      </template>
				</el-table-column>
		...
		methods:{
			mappInput(){
				...
				axios({
					...
					console.log(message)
					that.mapp(message.data.result[0])
				})
			},
				 ...
				 axios({
					...
					for(let i=0;i<messData.length;i++){
                         switch (messData[i].id){
							 ...
							 break;
							 case '2005':that.con.waterPressure=(messData[i].value)/100;
							 that.con.waterPressure=((that.con.waterPressure-0.09)*2).toFixed(2);
							 if(that.con.waterPressure<0){
							 	that.con.waterPressure=0.00
							 }else{
							 	that.con.waterPressure=that.con.waterPressure
							 }
							 break;

							 case '2004':that.con.waterTemperature=(messData[i].value)/100
							 break;
							 case '1001':that.con.devStatus=messData[i].value
							 break;
							 case '1002':that.con.devIccid=messData[i].value
							 break;
							 case '2003':that.devLongitude=messData[i].value;
							 break;
							 case '2002':that.devLatitude=messData[i].value;
							 break;
							 default:break;
						 }
					}
					...
					if(that.devLongitude != "" && that.devLatitude != ""){
						...
						if(that.con.devStatus=='异常'){
							...
							var html="<table border='1' style='border:1px solid #e6e6e6;margin-top:20px'>"
								...
								+"<td style='width:90px;height:25px;line-height:25px;'><img style='width:15px;margin-right:5px;' src='./static/image/icon/battery (8).png' alt=''>设备水压</td><td style='width:200px;height:25px;line-height:25px;'>"+that.con.waterPressure+'MPa'+"</td></tr>"
					...
					if(message.data.resCode==0){
								...
								data:Qs.stringify(datap)
									...
									for(let i=0;i<messData.length;i++){
										switch (messData[i].id){
											...
											break;
											 case '1003':that.con.devParaSn=messData[i].value
											...
											break;
											case '1002':that.con.devIccid=messData[i].value
													localStorage.setItem('morethros',(messData[i].value));
											break;
											case '2003':that.ccc=messData[i].value
												localStorage.setItem("bbb",messData[i].value)
												that.posac=that.ccc
												console.log(that.posac)
											break;
											case '2002':that.bbb=messData[i].value
												localStorage.setItem("ccc",messData[i].value)
												that.abad=that.bbb
							...
							setTimeout(function(){
								...
								if(that.five=='异常'){
									...
									+"<td style='width:90px;height:25px;line-height:25px;'><img style='width:15px;margin-right:5px;' src='./static/image/icon/battery (8).png' alt=''>设备水压</td><td style='width:200px;height:25px;line-height:25px;'>"+that.seven+'MPa'+"</td></tr>"
						...
						if(that.$route.query.mouse==undefined||that.$route.query.mouse=='[object Object]'){
							...
							that.seven=((that.seven-0.09)*2).toFixed(2);
							 if(that.seven<0){
							 	that.seven=0.00
							 }else{
							 	that.seven=that.seven
							 }
