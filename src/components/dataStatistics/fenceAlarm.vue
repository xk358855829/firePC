	...
	<div id="fenceAlarm">
		...
		<div class="Content">
			<el-table class="oyTable" :data="tableData" style="width: 100%">
				...
				<el-table-column label="围栏名字" width="150">
					<template slot-scope="scope">
				        <el-popover trigger="hover" placement="top">
				        	<p  class="TsMaxWidth">{{ scope.row.enclName }}</p>
				        	<div slot="reference" class="name-wrapper">
				        		<div class="overflowHide">{{ scope.row.enclName }}</div>
						   	</div>
					    </el-popover>
					</template>
				</el-table-column>
				...
				<el-table-column label="状态" width="200">
					<template slot-scope="scope">
						<el-popover trigger="hover" placement="top">
						    <p  class="TsMaxWidth">{{ scope.row.enclPolice}}</p>
						   	<div slot="reference" class="name-wrapper">
								<div class="overflowHide">{{ scope.row.enclPolice }}</div>
						    </div>
						</el-popover>
					</template>
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			searchClick(){
				...
				axios({
					method:'post',
					url:this.$store.state.http +'/Statistics/EncloseCon',
